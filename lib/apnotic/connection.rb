...
module Apnotic
  ...
  class Connection
    ...
    def initialize(options={})
      ...
      if @cert
        # cert passed in as string
      elsif @cert_path && (@cert_path.respond_to?(:read) || File.exist?(@cert_path))
        # have cert
      else
        raise "Cert file not found: #{@cert_path}"
      end
    ...
    def certificate
      @certificate ||= begin
        if @cert
          cert = @cert
        elsif @cert_path.respond_to?(:read)
          cert = @cert_path.read
